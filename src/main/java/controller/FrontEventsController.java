...
import service.PlaceholderImageService;
...
public class FrontEventsController implements Initializable {
    ...
    private static final String[] GENRE_COLORS = {
        "#E8320A","#6366F1","#10B981","#F59E0B","#8B5CF6","#0EA5E9","#F43F5E","#14B8A6","#EC4899","#84CC16"
    };

    private String genreColor(String genre) {
        if (genre == null || genre.isBlank()) return "#6366F1";
        return GENRE_COLORS[Math.abs(genre.hashCode()) % GENRE_COLORS.length];
    }
    ...
    private VBox buildEventRow(Event event) {
        ...
        String gColor = genreColor(event.getGenre());
        Region accentBar = new Region();
        accentBar.setPrefWidth(4);
        accentBar.setMinWidth(4);
        accentBar.setStyle("-fx-background-color:" + gColor + ";");

        HBox inner = new HBox(20);
        inner.setAlignment(Pos.CENTER_LEFT);
        inner.getStyleClass().add("event-list-inner");

        HBox rowWrap = new HBox(0, accentBar, inner);
        ...
        // Event thumbnail (shown if image exists)
        if (event.getImageFilename() != null && !event.getImageFilename().isBlank()) {
            java.io.File imgFile = new java.io.File("uploads", event.getImageFilename());
            if (imgFile.exists()) {
                ImageView thumb = new ImageView(
                    new Image(imgFile.toURI().toString(), 110, 90, true, true, true));
                thumb.setFitWidth(110); thumb.setFitHeight(90);
                thumb.setPreserveRatio(false);
                thumb.setStyle("-fx-background-radius:10; -fx-effect:dropshadow(gaussian,rgba(0,0,0,0.35),8,0,0,2);");
                inner.getChildren().add(0, thumb);
            }
        }
        ...
        // Artist avatar (async, shown if artistId is set)
        StackPane avatarWrap = new StackPane();
        avatarWrap.setMinSize(52, 52);
        avatarWrap.setPrefSize(52, 52);
        avatarWrap.setMaxSize(52, 52);
        Region avatarBg = new Region();
        avatarBg.setPrefSize(52, 52);
        avatarBg.setStyle("-fx-background-color:" + gColor + "44;-fx-background-radius:26;");
        Label avatarFallback = new Label("🎵");
        avatarFallback.setStyle("-fx-font-size:20px;");
        ImageView avatarImg = new ImageView();
        avatarImg.setFitWidth(52); avatarImg.setFitHeight(52);
        Circle avatarClip = new Circle(26, 26, 26);
        avatarImg.setClip(avatarClip);
        avatarWrap.getChildren().addAll(avatarBg, avatarFallback, avatarImg);

        if (event.getArtistId() != null) {
            new Thread(() -> {
                try {
                    User artist = new UserService().recupererParId(event.getArtistId());
                    if (artist != null) {
                        String name = artist.getPrenom() + " " + artist.getNom();
                        String color = gColor.replace("#", "");
                        String url = PlaceholderImageService.userAvatar(name, color);
                        Platform.runLater(() -> {
                            PlaceholderImageService.loadInto(avatarImg, url, null);
                            avatarImg.imageProperty().addListener((o, old, n) -> {
                                if (n != null && !n.isError()) avatarFallback.setVisible(false);
                            });
                        });
                    }
                } catch (Exception ignored) {}
            }, "artist-avatar-" + event.getId()).start();
        }

        right.getChildren().add(btn);
        inner.getChildren().addAll(dateBlock, info, avatarWrap, right);
        row.getChildren().add(rowWrap);
        return row;
    }
